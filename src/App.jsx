...
const MORNING_BRIEFING_SYSTEM = (profile, tasks, memoryContext, dayInfo) => {
  const conditionStyle = {
    ...
    depression: "Be warm and gentle. Keep expectations low and achievable. Celebrate that they showed up today. One tiny thing is enough.",
  ...
  return `You are Luma generating a personalized morning briefing. This is the most important message of the user's day — it sets their tone, focus, and energy.

Style guide for this user: ${style}

Format the briefing like this — warm, personal, never robotic:

Start with a warm personal greeting using their name and the day/date.

Then include these sections naturally — not as rigid headers but as flowing, caring paragraphs:

1. A genuine check-in question — ask how they're feeling today (1 question only)
2. Today's focus — 1 to 3 things to focus on based on their goals and past conversations
3. Tasks — acknowledge their pending tasks gently, help them pick just ONE to start with
4. A personal insight — something encouraging based on what you know about them
5. One gentle encouragement — a single sentence that feels human and real

Keep the whole briefing under 250 words. Make it feel like a text from a caring friend, not a productivity app.

User: ${profile.name || "friend"}
Day: ${dayInfo.dayName}, ${dayInfo.date}
Time: ${dayInfo.timeOfDay}
Goals: ${profile.goals || "not set yet"}
Career: ${profile.career || "not set yet"}
Condition: ${profile.condition && profile.condition !== "none" ? profile.condition : "none"}

Pending tasks:
${pendingTasks || "No tasks yet"}

${memoryContext ? `Recent context from past conversations:\n${memoryContext}` : ""}

Generate the morning briefing now. Make it feel genuinely personal and warm.`;
};
...
export default function App() {
  ...
  useEffect(() => {
    if (!showOnboarding && profile.name && shouldShowBriefing() && ollamaStatus === "online") {
      generateBriefing();
    }
    ...
    try {
      const res = await fetch(OLLAMA_URL, {
        ...
        body:JSON.stringify({
          ...
          messages:[{ role:"user", content:MORNING_BRIEFING_SYSTEM(profile, tasks, memoryContext, dayInfo) }],
          stream:false
        })
      });
      const data = await res.json();
      const content = data.message?.content || "Good morning! I'm here whenever you're ready.";
      setBriefing(content);
      localStorage.setItem("luma_last_briefing", new Date().toISOString());
    } catch { setBriefing("Good morning! Couldn't connect to generate your briefing — but I'm here when you need me."); }
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const res = await fetch(OLLAMA_URL, {
        ...
        body:JSON.stringify({
          model:"mistral-nemo:latest",
          messages:[{ role:"system", content:LUMA_SYSTEM(profile, memoryContext) }, ...updated.slice(-MAX_MEMORY_MESSAGES)],
          stream:false
        })
      });
      const data = await res.json();
    ...
    <div style={{ minHeight:"100vh", background:"#f5f2ee", fontFamily:"'Nunito',-apple-system,sans-serif", color:"#1a1a1a", position:"relative", overflow:"hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .briefing-card{animation:briefingIn 0.5s cubic-bezier(0.34,1.2,0.64,1);}
      ...
      {screen==="home"&&(
        <div className="section-scroll" style={{ paddingTop:"92px", position:"relative", zIndex:1, minHeight:"100vh" }}>
          <div style={{ maxWidth:"720px", margin:"0 auto" }}>
            ...
            {showBriefingCard&&(
              <div className="briefing-card" style={{ background:"rgba(255,255,255,0.82)", border:"0.5px solid rgba(26,26,26,0.07)", borderRadius:"28px", padding:"28px 32px", marginBottom:"28px", backdropFilter:"blur(20px)", boxShadow:"0 4px 32px rgba(0,0,0,0.06)", position:"relative" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"16px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                    ...
                    <div style={{ fontSize:"12px", fontWeight:800, color:"rgba(26,26,26,0.35)", letterSpacing:"0.08em", textTransform:"uppercase" }}>
                      {dayInfo.dayName} Morning Briefing
                    </div>
                  </div>
                  <button onClick={()=>setBriefingDismissed(true)} style={{ background:"none", border:"none", fontSize:"18px", color:"rgba(26,26,26,0.25)", cursor:"pointer", lineHeight:1, padding:"0 4px" }}>×</button>
                </div>

                {briefingLoading ? (
                  <div>
                    <div style={{ height:"18px", borderRadius:"8px", marginBottom:"10px", width:"75%" }} className="briefing-shimmer" />
                    <div style={{ height:"14px", borderRadius:"8px", marginBottom:"8px", width:"90%" }} className="briefing-shimmer" />
                    <div style={{ height:"14px", borderRadius:"8px", marginBottom:"8px", width:"65%" }} className="briefing-shimmer" />
                    <div style={{ height:"14px", borderRadius:"8px", marginBottom:"8px", width:"80%" }} className="briefing-shimmer" />
                    <div style={{ fontSize:"13px", fontWeight:600, color:"rgba(26,26,26,0.3)", marginTop:"16px" }}>Generating your morning briefing…</div>
                  </div>
                ) : (
                  <div>
                    <div style={{ fontSize:"16px", fontWeight:500, color:"#1a1a1a", lineHeight:1.72, whiteSpace:"pre-wrap", marginBottom:"20px" }}>
                      {briefing}
                    </div>
                    <div style={{ display:"flex", gap:"10px" }}>
                      <button onClick={respondToBriefing} style={{ padding:"11px 22px", background:"linear-gradient(135deg,#00c896,#0ea5e9)", border:"none", borderRadius:"14px", color:"#fff", fontSize:"14px", fontWeight:800, cursor:"pointer", fontFamily:"'Nunito',sans-serif", transition:"all 0.2s" }}>
                        Respond to Luma
                      </button>
                      <button onClick={()=>{generateBriefing();}} style={{ padding:"11px 18px", background:"rgba(26,26,26,0.05)", border:"0.5px solid rgba(26,26,26,0.08)", borderRadius:"14px", color:"rgba(26,26,26,0.5)", fontSize:"14px", fontWeight:700, cursor:"pointer", fontFamily:"'Nunito',sans-serif" }}>
                        Refresh
                      </button>
                      <button onClick={()=>setBriefingDismissed(true)} style={{ padding:"11px 18px", background:"transparent", border:"none", color:"rgba(26,26,26,0.3)", fontSize:"14px", fontWeight:700, cursor:"pointer", fontFamily:"'Nunito',sans-serif" }}>
                        Dismiss
                      </button>
                    </div>
                  </div>
            ...
            {/* Last session recall — only show if no briefing */}
            ...
            <div style={{ animation:"fadeUp 0.6s ease 0.2s both" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px" }}>
                ...
                {briefingDismissed&&(
                  <button onClick={()=>{setBriefingDismissed(false);generateBriefing();}} style={{ fontSize:"12px", fontWeight:800, color:"rgba(26,26,26,0.3)", background:"none", border:"none", cursor:"pointer", fontFamily:"'Nunito',sans-serif" }}>
